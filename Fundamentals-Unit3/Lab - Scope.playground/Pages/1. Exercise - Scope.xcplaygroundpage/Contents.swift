...
//print("The value of foo is \(foo)")
//variable "foo" is declared inside the for loop and has a limited scope. It is only accessible within the for loop and cannot be referenced outside of it. Attempting to access "foo" outside of the loop will result in a compiler error because it is not in scope.
...
//ly because "x" is defined outside of the for loop in the global scope. This means it can be accessed and modified within the for loop as well as outside of the loop. On each iteration of the for loop, the value of "x" is incremented and printed, and after the loop has completed, the final value of "x" is also printed. In contrast, "foo" was defined inside the for loop in the previous example and was only accessible within the for loop, but not outside of it.
