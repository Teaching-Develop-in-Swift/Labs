...
func getMeal(meal: String) -> Meal? {
    guard let meal = meals[meal] else { return nil }
    return meal
}
 ...
 */
func checkMeal() -> [String: Any]? {
    guard let data = UserDefaults.standard.dictionary(forKey: "mealLog") else { return nil }
    return data
}
if let dict = checkMeal() {
    print(dict)
}
/*:
