...
//error will be generated because foo is only recognised within the for-loop curly brackets (scope)
...
// x is defined outside the for-loop scope i.e. it's defined globably, and it was modified within for-loop scope which is fine as it's already defined outside it as described. on the contrary, foo was defined inside for-loop scope so it can't be accessed from outside
...
greeting(greeting: "Hi there", name: "asd")
greeting(greeting: nil, name: "sakr")
