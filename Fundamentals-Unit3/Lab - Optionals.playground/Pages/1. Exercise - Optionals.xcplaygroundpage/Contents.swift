...
print("If we did not define userAge with optionals we got 'Value of optional type 'Int?' must be unwrapped to a value of type 'Int'' error.")
...
print(userAge)
print("When we try parsing one type to another and if this value which is we try to parse is not a correct format we get nil value.")
/*:
