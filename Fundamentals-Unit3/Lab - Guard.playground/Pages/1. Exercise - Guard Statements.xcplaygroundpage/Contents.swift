...
let positiveExample = calculateArea(x: 5, y: 2)
print(positiveExample)
let negativeExample = calculateArea(x: 6, y: -2)
print(negativeExample)
...
func add(x: Int?, y: Int?) -> Int? {
    guard let firstArg = x, let secondArg = y else { return nil }
    return firstArg + secondArg
}
let result = add(x: 2, y: 2)
print(result)
let nilExample = add(x: nil, y: 2)
print(nilExample)
/*:
...
func createUser() -> User? {
    guard let name = firstNameTextField.text, let lastName = lastNameTextField.text, let age = ageTextField.text else {return nil}
    return User(firstName: name, lastName: lastName, age: age)
}
