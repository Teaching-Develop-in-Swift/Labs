...
let heartRate2D = 88.0
let heartRate3D = 92.0
let addedHRD = heartRate1D + heartRate2D + heartRate3D
let averageHRD = addedHRD / 3
print(averageHRD)
//The result is different from above example because above example using int constants for calculations. For this reason if the division result is 'double', but we try to divide two integer value, swift gives floor the result.
