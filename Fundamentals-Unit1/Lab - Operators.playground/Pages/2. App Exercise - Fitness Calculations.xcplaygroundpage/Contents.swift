...
let heartRate3D: Double = 88
let addedHRD = heartRate1D + heartRate2D + heartRate3D
let averageHRD = addedHRD / 3
print(averageHRD)

/// In the first case, the average is obtained starting from an Int values, so the result is of type Int. In the second case, we forced types to be Double, so the results will be Double as well.
